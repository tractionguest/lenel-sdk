...
module SwaggerClient
  class BadgePrintersBody1
    # The ID associated with a given badge type.
    attr_accessor :badge_type_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::BadgePrintersBody1` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::BadgePrintersBody1`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'badge_type_id')
        self.badge_type_id = attributes[:'badge_type_id']
      end
      ...
      if attributes.key?(:'printer_name')
        self.printer_name = attributes[:'printer_name']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @badge_type_id.nil?
        invalid_properties.push('invalid value for "badge_type_id", badge_type_id cannot be nil.')
      end

      if @printer_name.nil?
        invalid_properties.push('invalid value for "printer_name", printer_name cannot be nil.')
      end
    ...
    def valid?
      return false if @badge_type_id.nil?
      return false if @printer_name.nil?
      true
    end
    ...
    def hash
      [badge_type_id, workstation, printer_name].hash
    end
