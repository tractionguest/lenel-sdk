  ...
  class CardholderOptions
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::CardholderOptions` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::CardholderOptions`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [activate_deactivate_dates_use_time, badge_pin_properties, create_photo_thumbnails, max_accesslevels_per_badge_standard, max_accesslevels_per_badge_temporary, max_accesslevels_per_badge_total, max_active_badges, max_badge_id_length, max_extended_id_length, temporary_accesslevel_granularity, version].hash
    end
