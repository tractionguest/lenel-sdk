...
module SwaggerClient
  class DirectoryAccountsMatchingCardholdersSuccessfulListItemList
    attr_accessor :cardholder_id

    attr_accessor :directory_account
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::DirectoryAccountsMatchingCardholdersSuccessfulListItemList` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::DirectoryAccountsMatchingCardholdersSuccessfulListItemList`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'cardholder_id')
        self.cardholder_id = attributes[:'cardholder_id']
      end

      if attributes.key?(:'directory_account')
        self.directory_account = attributes[:'directory_account']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          cardholder_id == o.cardholder_id &&
          directory_account == o.directory_account
    end
    ...
    def hash
      [cardholder_id, directory_account].hash
    end
