  ...
  class GetUserPreferences
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::GetUserPreferences` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::GetUserPreferences`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'preference_list')
        if (value = attributes[:'preference_list']).is_a?(Array)
          self.preference_list = value
        end
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          preference_list == o.preference_list &&
    ...
    def hash
      [total_items, preference_list, version].hash
    end
