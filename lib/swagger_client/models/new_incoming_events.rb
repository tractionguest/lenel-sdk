...
module SwaggerClient
  class NewIncomingEvents
    attr_accessor :incoming_event_list

    # Attribute mapping from ruby-style variable name to JSON key.
    def self.attribute_map
      {
        :'incoming_event_list' => :'incoming_event_list'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::NewIncomingEvents` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::NewIncomingEvents`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'incoming_event_list')
        if (value = attributes[:'incoming_event_list']).is_a?(Array)
          self.incoming_event_list = value
        end
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          incoming_event_list == o.incoming_event_list
    end
