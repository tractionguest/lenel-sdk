...
module SwaggerClient
  class UpdateCardholderWithDirectoryAccountProperty
    attr_accessor :property_written
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::UpdateCardholderWithDirectoryAccountProperty` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::UpdateCardholderWithDirectoryAccountProperty`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'property_written')
        self.property_written = attributes[:'property_written']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          property_written == o.property_written &&
