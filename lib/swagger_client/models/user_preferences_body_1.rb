  ...
  class UserPreferencesBody1
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::UserPreferencesBody1` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::UserPreferencesBody1`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'setting_type')
        self.setting_type = attributes[:'setting_type']
      end
    ...
    def list_invalid_properties
      ...
      if @setting_type.nil?
        invalid_properties.push('invalid value for "setting_type", setting_type cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @setting_type.nil?
      true
    end
    ...
    def hash
      [client_name, preference_settings, setting_type, is_global].hash
    end
